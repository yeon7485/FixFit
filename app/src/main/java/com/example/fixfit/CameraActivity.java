...
import com.example.fixfit.env.ImageUtils;
...
import com.example.fixfit.tflite.Classifier.Recognition;
...
public abstract class CameraActivity extends AppCompatActivity
    ...
    protected void onCreate(final Bundle savedInstanceState) {
        ...
        setContentView(R.layout.tech_neck_realtime);
        ...
        result_text = findViewById(R.id.pose_text);
                ...
                if (recognition.getConfidence() > 0.90f) {
                    ...
                    if (flag == 0) {  // 현재상태 good일때
                        ...
                        if (!(recognition.getTitle().equals("bad_left") || recognition.getTitle().equals("bad_right"))) {  //good으로 바뀐다면 (bad가 아니라면, 식별 불가일때 포함하기 위함)
                            flag = 0;
                            result_text.setText("자세 좋음");
                            result_text.setBackgroundColor(Color.argb(0xaa, 0x00, 0xaa, 0x00));
                            if (startTime != 0) {
                                //endAlarm();
                            }
                        }
